...
const SendQuery = ({ onSendMessage, disabled = false, fileInputRef: externalFileInputRef, onRowsChange }) => {
  ...
  const isUploadingAttachment = useChatStore((state) => state.isUploadingAttachment);
  const clearPendingAttachments = useChatStore((state) => state.clearPendingAttachments);
  const currentSession = useChatStore((state) => state.currentSession);
  const markSessionAsInitiated = useChatStore((state) => state.markSessionAsInitiated);
  ...
  const handleSend = async () => {
    if (inputValue.trim() && onSendMessage && !disabled && !isUploadingAttachment) {
      // Check if the last message is from the user (indicating no response was received)
      if (messages.length > 0 && messages[messages.length - 1].role === "user") {
        console.log("[SendQuery] Last message is from user, adding error message");
  ...
  return (
    <div className="w-full px-4 py-3 flex items-center gap-2 bg-gray-100" style={{ maxWidth: '100%' }}>
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
      ...
      >
        ...
        <button 
          ...
          className={`flex-shrink-0 transition-all duration-200 rounded-lg p-1 ${
